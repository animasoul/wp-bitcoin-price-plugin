    ...
    bitcoinBlocks.forEach(block => {
      CURRENCIES.forEach(currency => {
        ...
        if (data.bpi[currency] && ratePlaceholder) {
          updateRateClass(ratePlaceholder, data.bpi[currency].rate_float, previousRates[currency]);
          setTimeout(() => {
      ...
      const disclaimerPlaceholder = block.querySelector(".disclaimer-placeholder");
      if (data.disclaimer && disclaimerPlaceholder) {
        disclaimerPlaceholder.textContent = data.disclaimer;
      }
      const updatedTimePlaceholder = block.querySelector(".updated-time-placeholder");
